...
class assignment3 
{
    ...
    public static float[] updatePageRank(float[] webpageRank, float newValue) 
    {
        for (int i = 0; i < 100; i++) 
        {
            webpageRank[i] = newValue;
    ...
    {
        if (numUniqueLinks == 0)
            return dampingFactor * oldPageRank / 100.0f;

        return dampingFactor * (oldPageRank / (float) numUniqueLinks);
