    ...
    public static void updatePageRank(int[][] A, float[] webpageRank, float dampingFactor, double convergenceCriteria) 
    {
        try 
        {
            ...
            while (!hasConverged) 
            {
                ...
                List<Map.Entry<Integer, Float>> output = new LinkedList(positionRank.values());

                Collections.sort(output, new Comparator<Float>() 
                {
                    @Override
                ...
                for (int i = 0; i < 99; i++) 
                {
                    writer.write(String.valueOf(i + 1) + "(" + webpageRank[i] + "), ");
                }
